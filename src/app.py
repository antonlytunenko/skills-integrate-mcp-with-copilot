...
# In-memory authentication tokens (maps token -> username)
auth_tokens = {}
...
def login(username: str, password: str):
    ...
    # Check credentials against teachers database
    for teacher in teachers_db["teachers"]:
...
def verify_teacher_token(token: str):
    """Helper function to verify a teacher's authentication token"""
    if token not in auth_tokens:
        raise HTTPException(status_code=401, detail="Authentication required. Please log in as a teacher.")
